    ...
    <div class="text-center py-12 lg:py-16 min-h-screen bg-slate-50 px-[5vw]">
      ...
      >
        ...
        <FormStep>
          ...
          <CheckboxSet isExclusive options={['yes', 'maybe', 'no']} name="feedback" />
        </FormStep>
      </TheForm>
    </div>
  )
...
function CheckboxSet(
  props: JSX.InputHTMLAttributes<HTMLInputElement> & { options: string[]; isExclusive?: boolean },
) {
        ...
        {(option, i) => (
          <label for={getId(i())} class="w-fit">
            <input
              ...
              value={option}
            />
            <span class="rounded-full flex items-center justify-center border-2 border-transparent peer-checked:bg-sky-100 peer-checked:border-sky-400 transition-colors cursor-pointer px-2 py-1 text-sm peer-focus-visible:border-blue-500">
              {option}
            </span>
          </label>
        )}
...
function InputField(
  props: JSX.InputHTMLAttributes<HTMLInputElement> & { label: string; name: string },
) {
  const [local, other] = splitProps(props, ['name', 'label'])

  return (
    <div class="py-2 w-64 mx-auto">
      <label for={local.name} class="text-left text-sm mb-1 block">
        {local.label}
      </label>
