...
export function createMultiform(
  handleData: (data: FormData) => Promise<boolean>,
  autoValidate = true,
  ...
  function TheForm(props: TheFormProps) {
    return (
      <Show
        when={response() === null}
        fallback={
          <Show when={response()} fallback={props.failed || <p>Failed to send message.</p>}>
            {props.suceeded || <p>Thank You for Your message</p>}
          </Show>
        }
      >
        <form use:formHandler class={props.formStyle || ''}>
          ...
          <div class={props.btnWrapperStyle || ''}>
            ...
            <button disabled={isSent()} class={props.mainBtnStyle || props.btnStyle || ''}>
              {isSent()
  ...
  function FormStep(props: { children: JSX.Element }) {
    ...
    return <Show when={step() === thisStep}>{props.children}</Show>
  }
...
export function sendFormData(fetchEndpoint: string) {
  return async (formData: FormData) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Network response for multipart/form-data fetch was not OK. Status code: ${response.status}`,
        )
      }
...
export function ValidatedInput(props: FieldProps & JSX.InputHTMLAttributes<HTMLInputElement>) {
  ...
  const [named, others] = splitProps(props, [
    'inputStyle',
    'invalidStyle',
    'errorMessageStyle',
    'checkOnBlur',
  ...
  return (
    <>
      <input
        ...
        onInvalid={(e: { currentTarget: HTMLInputElement }) =>
          setError(e.currentTarget.validationMessage)
        }
        onBlur={
          named.checkOnBlur
            ? (e: { currentTarget: HTMLInputElement }) => e.currentTarget.checkValidity()
            : undefined
        }
        onInput={(e: { currentTarget: HTMLInputElement }) =>
          e.currentTarget.validity.valid && setError('')
        }
        class={`${named.inputStyle || ''} ${
          error() ? named.invalidStyle || '' : named.defaultStyle || ''
        }`}
        {...others}
      />
      <Show when={error()}>
